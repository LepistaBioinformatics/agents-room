...
export function TrashPanel({ items, onClose, onRestore, onDelete }: Props): JSX.Element {
  ...
  const handleRestore = async (id: string): Promise<void> => {
    ...
    setBusy(null)
    if (result.needsNewLocation) alert(t('trash.restoreError', { name: result.itemName }))
  }
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-ag-border/60 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold text-ag-text-1">{t('trash.title')}</span>
          ...
          {items.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead className="border-b border-ag-border/60 bg-ag-surface-2/60">
                <tr>
                  {[t('trash.columns.name'), t('trash.columns.type'), t('trash.columns.workspace'), t('trash.columns.trashed'), t('trash.columns.actions')].map((h, i) => (
                ...
                {items.map((item) => (
                  <tr key={item.id} className="hover:bg-ag-surface-2/40">
                    ...
                    <td className="px-3 py-3 text-xs text-ag-text-3 max-w-[180px] truncate">
                      {item.workspacePath || t('trash.global')}
                    </td>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        {confirmDeleteId === item.id ? (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-red-500">{t('trash.permanentlyDelete')}</span>
                            <button
